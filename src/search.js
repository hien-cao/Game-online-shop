'use strict';
/*
    The search.js handles the search engine for the games in the web app.
    The game can be sought by name, categories, and developer name.
    The suggestions  are provided based on the current input in the search box.
...
(() => {
  ...
  const search = debounced((event) => {
    ...
    let query; const q = event.target.value;
    if (q[0] == '#') {
  ...
  function autosuggestion(query, namelist) {
    ...
    /* The autosuggestion function takes two arguments, the input value and a
      * list of possible autosuggested values */
    // Create currentFocus to define the position of suggested
    // item to add class active and simulate a click event
    ...
    // Close all the suggestion list when someone clicks in the document
    // except clicking on the suggestion list itself
